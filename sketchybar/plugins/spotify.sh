...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set $NAME icon.color=0xff1bd760 label.drawing=off icon.drawing=off
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${ARTIST} - ${TRACK}" label.drawing=off icon.drawing=on icon.color=0xffa6da95 label.drawing=off
        sketchybar --set sep4 label.drawing=off

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME icon.color=0xff989898 label.drawing=off icon.drawing=on
    elif [ $PLAYER_STATE = "Stopped" ]; then
        sketchybar --set $NAME icon.color=0xff989898 label.drawing=off icon.drawing=off
        sketchybar --set sep4 label.drawing=off
    else
        sketchybar --set $NAME icon.color=0xff989898 label.drawing=off
    fi
}
