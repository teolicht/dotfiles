...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set "$NAME" icon.color="$SPOTIFY_GREEN" label.drawing=off icon.drawing=off
        return
    fi
    ...
    if [ "$PLAYER_STATE" = "Playing" ]; then
        ...
        sketchybar --set "$NAME" label="${ARTIST} - ${TRACK}" label.drawing=off icon.drawing=on icon.color="$SPOTIFY_GREEN" label.drawing=off

    elif [ "$PLAYER_STATE" = "Paused" ]; then
        sketchybar --set "$NAME" icon.color="$T_LIGHT_GRAY" label.drawing=off icon.drawing=on
    elif [ "$PLAYER_STATE" = "Stopped" ]; then
        sketchybar --set "$NAME" icon.color="$T_LIGHT_GRAY" label.drawing=off icon.drawing=off
    else
        sketchybar --set "$NAME" icon.color="$T_LIGHT_GRAY" label.drawing=off
    fi
}
